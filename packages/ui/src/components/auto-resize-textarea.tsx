 ...
 * Promovido pra `@papopro/ui` em M5#5 — antes vivia em `features/inbox/`
 * porque só o composer consumia. Hoje (M5#5) o chat de simulação do editor
 * de agentes também precisa do mesmo comportamento.
 *
 * UX que vendedor espera de WhatsApp Web, iMessage, Slack: input fica de 1
 * linha quando vazio, expande conforme digita, máximo de 6 linhas pra não
 * esconder a thread.
 *
 * Implementação:
 ...
 *
 * Estilos seguem [textarea.tsx](./textarea.tsx) (border, focus ring,
 * aria-invalid) — visual idêntico ao Textarea da DS.
 */
...
export const AutoResizeTextarea = React.forwardRef<HTMLTextAreaElement, AutoResizeTextareaProps>(
  function AutoResizeTextarea({ className, value, onChange, ...props }, ref) {
    ...
    // `useImperativeHandle` (consumers podem precisar do ref pra inserir tokens).
